...
import { readSession, issueSubmission } from './lib/session.mjs';
...
export default endpoint(async request => {
  ...
  if (request.method === 'POST') {
    ...
    try { return json({ ...calculate(data.selectedIds,catalog), submissionToken:issueSubmission() }); }
    catch { throw new HttpError(422,'INVALID_SELECTION','The selection contains an invalid module. Reload the proposal.'); }
  }
  return json({ catalog, version: catalogVersion, submissionToken: issueSubmission() });
});
