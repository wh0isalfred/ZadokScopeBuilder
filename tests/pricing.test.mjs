...
describe('authoritative pricing', () => {
 it('calculates all launch modules', () => expect(calculate(catalog.filter(x=>x.category!=='addition').map(x=>x.id),catalog).total).toBe(300000));
 it('calculates all possible additions with dependencies', () => expect(calculate(catalog.map(x=>x.id),catalog).total).toBe(595000));
 ...
 it('deduplicates IDs and includes foundation once', () => expect(calculate(['catalogue','catalogue','foundation'],catalog).total).toBe(137000));
...
it('keeps monthly care outside all one-time calculations', async () => {
 ...
 expect(quote.total).toBe(167000);
