...
import quotation from '../netlify/functions/quotation.mjs';
import pdf from '../netlify/functions/quotation-pdf.mjs';
import { createQuotation } from '../netlify/functions/lib/quotation.mjs';
import { issueSession, sign } from '../netlify/functions/lib/session.mjs';
const url='https://zadok-scope.netlify.app';
function request(path,data,extra={}){return new Request(`${url}/${path}`,{method:data?'POST':'GET',headers:{origin:url,'content-type':'application/json',cookie:`zadok_session=${issueSession()}`,...extra},...(data?{body:JSON.stringify(data)}:{})});}
